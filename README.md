...
- iPhone Safari 和“添加到主屏幕”后的网页 App 是两套本地存储。
- 如果桌面 App 打开后没有数据，先使用 `cloudflare-worker/mobile-bind-url.txt` 里的绑定地址重新打开并添加到主屏幕。
- 若 iOS 仍未带入绑定参数，桌面 App 会显示“绑定地址”输入框，把同一条绑定地址粘贴进去即可完成绑定。
